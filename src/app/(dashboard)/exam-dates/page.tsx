          ...
          <div className="bg-card border border-border rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-foreground">
                {editing ? "Edit Exam Date" : "New Exam Date"}
              </h2>
              <button
        ...
        <Table>
          <TableHeader className="bg-card">
            <TableRow className="hover:bg-transparent border-b border-border/50">
              ...
              <TableHead className="font-bold text-[11px] uppercase tracking-wider text-muted-foreground">Stream</TableHead>
              <TableHead className="font-bold text-[11px] uppercase tracking-wider text-muted-foreground">Registration</TableHead>
              <TableHead className="font-bold text-[11px] uppercase tracking-wider text-muted-foreground">Status</TableHead>
              <TableHead className="w-[80px] font-bold text-[11px] uppercase tracking-wider text-muted-foreground">Priority</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              filtered.map((exam) => (
                ...
                >
                  <TableCell className="font-semibold text-foreground text-[13px]">{exam.name}</TableCell>
                  <TableCell className="text-muted-foreground text-[13px]">{exam.examDate}</TableCell>
                  <TableCell className="text-muted-foreground text-[13px]">{exam.stream}</TableCell>
                  <TableCell className="text-muted-foreground text-[13px]">{exam.registration || "—"}</TableCell>
                  <TableCell>
                    <span className={`text-[10px] font-bold uppercase tracking-wide rounded-full border px-2.5 py-1 ${STATUS_STYLES[exam.status]}`}>
                      {exam.status}
                    </span>
                  </TableCell>
                  <TableCell className="text-muted-foreground text-[13px]">{exam.priority ?? 0}</TableCell>
              ...
              <TableStateRow colSpan={7} emptyLabel="No exam dates yet. Add one to get started." />
            )}
          </TableBody>
        </Table>
      </ListingLayout>
