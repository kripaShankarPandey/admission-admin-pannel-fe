...
const fileToBase64 = (file: File): Promise<string> =>
  uploadImage(file, "courses");

interface SubCategoryFormProps {
  courseId?: string | number;
}

// Doubles as the form's type: useForm infers its value shape from these.
const SUB_CATEGORY_DEFAULTS = {
  name: "",
  slug: "",
  courseCategoryId: "",
  priority: "1",
...
type SubCategoryFormValues = typeof SUB_CATEGORY_DEFAULTS;

/**
 * Shape of the free-form `details` JSON column on a sub-course category.
 * Every field is optional: rows written by older versions of this form, or by
 * the Excel import, may be missing any of them.
 */
interface CourseDetails {
  ...
  topIcons?: { name: string; value: string }[];
  banners?: {
    imageBase64: string;
    heading: string;
    info: string;
  ...
  overview?: {
    description?: string;
    careers?: { name: string; salary: string }[];
    topColleges?: {
      name: string;
      location: string;
      rating: string;
...
export function SubCategoryForm({ courseId }: SubCategoryFormProps) {
  ...
  const form = useForm({ defaultValues: SUB_CATEGORY_DEFAULTS });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await courseCategoryService.getAll({
          page: 1,
          pageSize: 100,
        });
    ...
    if (courseId) {
      ...
      subCourseCategoryService
        ...
        .then((res) => {
          ...
          try {
            details = JSON.parse(res.details || "{}") as CourseDetails;
          } catch (e) {
          ...
          reset({
            ...
            priority: details.priority?.toString() || "1",
            courses_image: res.courses_image || "",
            courseLevel: details.courseLevel || "",
            duration: details.duration || "",
            eligibility: details.eligibility || "",
            shortDescription: details.shortDescription || "",
            quickInfoItems: details.quickInfo?.items?.length
              ? details.quickInfo.items
              : [{ name: "", value: "" }],
            applyNowUrl: details.quickInfo?.applyNow || "",
            downloadBrochureUrl: details.quickInfo?.downloadBrochure || "",
            topIcons: details.topIcons?.length
              ? details.topIcons
              : [{ name: "", value: "" }],
            banners: details.banners?.length
              ? details.banners
              : [{ imageBase64: "", heading: "", info: "", url: "" }],
            overviewDescription: details.overview?.description || "",
            careers: details.overview?.careers?.length
              ? details.overview.careers
              : [{ name: "", salary: "" }],
            topColleges: details.overview?.topColleges?.length
              ? details.overview.topColleges
              : [
                  {
                    name: "",
                    location: "",
            ...
            keyFacts: details.keyFacts?.length
              ? details.keyFacts
              : [{ name: "" }],
            meta_title: details.seo?.meta_title || "",
            meta_description: details.seo?.meta_description || "",
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            {courseId
              ? `Edit Course: ${watch("name") || "Loading..."}`
              : "Create New Course"}
          </h1>
          <p className="text-sm text-muted-foreground">
            Fill in the comprehensive details for this course page.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div className="relative w-32 h-32 rounded-lg border border-border overflow-hidden group">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                        <Button
                          ...
                          variant="destructive"
                          size="sm"
                          onClick={() => setValue("courses_image", "")}
                        >
                          Remove
                        </Button>
                      </div>
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Upload a high-quality image representing this course.
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. MBA in Marketing" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="mba-in-marketing" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => {
                  ...
                  const options =
                    field.value && !LEVELS.includes(field.value)
                      ? [field.value, ...LEVELS]
                      : LEVELS;
                  return (
                    <FormItem>
          ...
          <Card className="shadow-sm border-border/50">
            <CardHeader className="bg-muted/20 border-b border-border/50 pb-4">
              ...
              <CardDescription>
                Links and key-value pairs shown at the top of the course page
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-3">
                ...
                {quickInfoFields.fields.map((item, index) => (
                  <div key={item.id} className="flex gap-3 items-start">
                    <FormField
                      control={control}
                      name={`quickInfoItems.${index}.name`}
                      render={({ field }) => (
                        <FormItem className="flex-1">
                          <FormControl>
                            <Input
                              placeholder="Name (e.g. Average Fee)"
                              {...field}
                            />
                          </FormControl>
                        </FormItem>
                    ...
                    <FormField
                      control={control}
                      name={`quickInfoItems.${index}.value`}
                      render={({ field }) => (
                        <FormItem className="flex-1">
                          <FormControl>
                            <Input
                              placeholder="Value (e.g. 1.2 Lakhs)"
                              {...field}
                            />
                          </FormControl>
                        </FormItem>
                    ...
                    <Button
                      ...
                      onClick={() => quickInfoFields.remove(index)}
                    >
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    quickInfoFields.append({ name: "", value: "" })
                  }
                >
          ...
          <Card className="shadow-sm border-border/50">
            <CardHeader className="bg-muted/20 border-b border-border/50 pb-4">
              ...
              <CardDescription>
                Highlights with icons/labels shown across the top header
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 pt-6">
              {topIconFields.fields.map((item, index) => (
                <div key={item.id} className="flex gap-3 items-start">
                  ...
                  <Button
                    ...
                    size="icon"
                    className="text-destructive shrink-0"
                    onClick={() => topIconFields.remove(index)}
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => topIconFields.append({ name: "", value: "" })}
              >
          ...
          <Card className="shadow-sm border-border/50">
            ...
            <CardContent className="space-y-6 pt-6">
              {bannerFields.fields.length === 0 && (
                <div className="text-center py-8 text-muted-foreground border-2 border-dashed border-border rounded-lg bg-muted/10">
                  No banners added yet. Click &quot;Add Banner&quot; above.
                </div>
              )}
              {bannerFields.fields.map((item, index) => (
                <div
                  key={item.id}
                  className="p-5 border border-border/60 rounded-xl space-y-4 relative bg-card shadow-sm hover:border-primary/30 transition-colors group"
                >
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative w-full aspect-video rounded-md border border-border overflow-hidden bg-muted/20 flex items-center justify-center">
                        {watch(`banners.${index}.imageBase64`) ? (
                          <>
                            <img
                              src={watch(`banners.${index}.imageBase64`)}
                              alt="Banner preview"
                              className="w-full h-full object-cover"
                            />
                              ...
                              <Input
                                ...
                                onChange={async (e) => {
                                  ...
                                  if (file) {
                                    const base64 = await fileToBase64(file);
                                    setValue(
                                      `banners.${index}.imageBase64`,
                                      base64,
                                    );
                                  }
                                }}
                              />
                              <span className="text-white text-xs font-semibold flex items-center gap-1 bg-black/40 px-3 py-1.5 rounded-full">
          ...
          <Card className="shadow-sm border-border/50">
            ...
            <CardContent className="space-y-10 pt-6">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        ...
                        placeholder="Detailed course overview. Use HTML or Markdown if supported..."
                        {...field}
                      />
                    </FormControl>
                  </FormItem>
              ...
              <div className="space-y-4 pt-4 border-t border-border/50">
                <FormLabel className="text-base font-semibold text-foreground flex justify-between items-center">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      careerFields.append({ name: "", salary: "" })
                    }
                    className="h-8"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {careerFields.fields.map((item, index) => (
                    <div
                      key={item.id}
                      className="flex gap-2 items-center p-2 rounded-lg border border-border/60 bg-muted/10"
                    >
                      <div className="flex-none text-muted-foreground/50 cursor-grab px-1">
                          ...
                          <FormItem className="flex-1 space-y-0">
                            <FormControl>
                              <Input
                                ...
                                className="h-9 outline-none border-none shadow-none bg-transparent"
                                {...field}
                              />
                            </FormControl>
                          </FormItem>
                      ...
                      <Button
                        ...
                        onClick={() => careerFields.remove(index)}
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border/50">
                ...
                {collegeFields.fields.length === 0 && (
                  <div className="text-sm text-muted-foreground italic">
                    No top colleges added.
                  </div>
                )}
                <div className="grid grid-cols-1 gap-4">
                  {collegeFields.fields.map((item, index) => (
                    <div
                      key={item.id}
                      className="p-5 border border-border/60 rounded-xl space-y-4 relative bg-card shadow-sm group"
                    >
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                        className="absolute top-3 right-3 h-8 w-8 text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-opacity"
                        onClick={() => collegeFields.remove(index)}
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mr-8">
                        ...
                        <FormField
                          control={control}
                          name={`topColleges.${index}.location`}
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-xs uppercase tracking-wider text-muted-foreground">
                                Location
                        ...
                        <FormField
                          control={control}
                          name={`topColleges.${index}.buttonUrl`}
                          render={({ field }) => (
            ...
            <CardContent className="space-y-4 pt-6">
              {keyFactFields.fields.map((item, index) => (
                <div key={item.id} className="flex gap-3 items-center">
                  <div className="flex-none text-muted-foreground/50 cursor-grab">
                    <GripVertical className="h-4 w-4" />
                  </div>
                  <FormField
                    control={control}
                    name={`keyFacts.${index}.name`}
                    render={({ field }) => (
                      <FormItem className="flex-1 m-0 space-y-0">
                        <FormControl>
                          <Input
                            placeholder="Key Fact (e.g. 100% Placement Assistance)"
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                  ...
                  <Button
                    ...
                    size="icon"
                    className="text-destructive shrink-0"
                    onClick={() => keyFactFields.remove(index)}
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => keyFactFields.append({ name: "" })}
              >
          ...
          <Card className="shadow-sm border-border/50">
            <CardHeader className="bg-muted/20 border-b border-border/50 pb-4">
              ...
              <CardDescription>
                Search engine metadata and live preview of how this page appears
                in Google
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="SEO title — keep under 60 characters"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-border/60 bg-background p-5">
                  ...
                  <div className="mb-4 flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-2">
                    ...
                    <div className="flex flex-1 items-center gap-2 rounded-md border border-border/40 bg-background px-3 py-1 text-xs text-muted-foreground">
                      ...
                      <span className="truncate font-mono text-[11px]">
                        google.com/search?q=
                        {encodeURIComponent(
                          metaTitleWatch || watch("name") || "course name",
                        )}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 pl-1">
                    ...
                    {/* Title */}
                    <p
                      className={`text-lg font-normal leading-snug ${metaTitleWatch || watch("name") ? "text-[#1a0dab]" : "text-muted-foreground/50 italic"} hover:underline cursor-pointer`}
                    >
                      {metaTitleWatch ||
                        (watch("name")
                          ? `${watch("name")} — Fees, Colleges, Career 2026 | Admission Today`
                          : "Meta title will appear here…")}
                    </p>

                    {/* Description */}
                    <p
                      className={`text-sm leading-relaxed ${metaDescriptionWatch ? "text-[#4d5156]" : "text-muted-foreground/40 italic"}`}
                    >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="rounded-lg border border-border/50 bg-muted/20 px-4 py-3">
                    <div className="mb-1.5 flex items-center justify-between">
                      ...
                      <span
                        className={`text-[11px] font-bold ${(metaTitleWatch?.length ?? 0) > 60 ? "text-red-500" : (metaTitleWatch?.length ?? 0) >= 50 ? "text-green-600" : "text-amber-500"}`}
                      >
                  ...
                  <div className="rounded-lg border border-border/50 bg-muted/20 px-4 py-3">
                    <div className="mb-1.5 flex items-center justify-between">
                      ...
                      <span
                        className={`text-[11px] font-bold ${(metaDescriptionWatch?.length ?? 0) > 160 ? "text-red-500" : (metaDescriptionWatch?.length ?? 0) >= 120 ? "text-green-600" : "text-amber-500"}`}
                      >
                    ...
                    <p className="mt-1.5 text-[10px] text-muted-foreground">
                      {(metaDescriptionWatch?.length ?? 0) > 160
                        ? "Too long — Google may truncate"
                        : (metaDescriptionWatch?.length ?? 0) >= 120
                          ? "Good length"
